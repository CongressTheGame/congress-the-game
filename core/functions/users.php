...
function change_profile_image($user_id, $file_temp, $file_extn){
        $query= mysql_query("SELECT profile FROM users WHERE user_id = $user_id");
...
function activated($email, $email_code) {
	$email	= mysql_real_escape_string($email);
	$email_code	= mysql_real_escape_string($email_code);

	return(mysql_result(mysql_query("SELECT COUNT('user_id') FROM `users` WHERE `email` = '$email' AND `email_code` = '$email_code' AND `active` = 1"),0) == 1) ? true : false;
}
...
function register_user($register_data)	{
	...
	email($register_data['email'], 'Activate your account.', "Hello " . $register_data['first_name'] . ",\n\nWelcome to Congress: the Game! Click on the following link to activate your account.\n\n http://www.congressthegame.com/activate.php?email=" . $register_data['email'] . "&email_code=" . $register_data['email_code'] ."\n\n- Philip");
